...
function endGame() {
  ...
  ROCKS.filter(e => { GAME.remove(e); return false; });
...
function moveDodgerLeft() {
   ...
   */
   ...
   return window.requestAnimationFrame(function step() {
     DODGER.style.left = `${(left > 0) ? left-=4 : left}px`;
...
function moveDodgerRight() {
   ...
   */
   var left = parseInt(DODGER.style.left);
   return window.requestAnimationFrame(function step() {
     DODGER.style.left = `${(left < 360) ? left+=4 : left}px`;
     //window.requestAnimationFrame(step);
   });
}
